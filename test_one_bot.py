...
if __name__ == '__main__':
    ...
    print("\n=== raw data detector ===\n")
    model_discriminator = BotGAT(cat_prop_size=config["discriminator"]["cat_prop_size"],
                            hidden_dim=config['discriminator']["embedding_dimension"]).to(other_device)
    ...
    test_discrim(model=model_discriminator,
                ...
                inputs=all_inputs)
    # exit()
    gpt_tweet_path = processed_data_folder + "id_tweet_vanilla.json"
    ...
    gpt_tweets_tensor=torch.load(gpt_tweets_tensor_path, weights_only=True).to(other_device)
