...
class Twibot22(Dataset):
    ...
    def tweets_embedding(self, tweets_path):
        print('Running feature2 embedding')
        path=self.root+tweets_path
        print(f"tweets path {path}")
        if not os.path.exists(path):
            ...
            tweets_tensor=torch.load(path, weights_only=True).to(self.device)
        # print('Finished')
    ...
    def train_val_test_mask(self):
        if self.root=='./Data/':
            ...
            train_idx=torch.load(self.root+'train_idx.pt', weights_only=True)
            val_idx=torch.load(self.root+'val_idx.pt', weights_only=True)
            test_idx=torch.load(self.root+'test_idx.pt', weights_only=True)
            
        return train_idx,val_idx,test_idx
        
        
    def dataloader(self, tweets_path):
        labels=self.load_labels()
        #self.Des_Preprocess()
        des_tensor=self.Des_embbeding()
        #self.tweets_preprocess()
        tweets_tensor=self.tweets_embedding(tweets_path)
