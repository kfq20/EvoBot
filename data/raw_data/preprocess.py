...
emoji_pattern = re.compile(
            "[\U0001F600-\U0001F64F]"  # 表情符号 (笑脸等)
            "|[\U0001F300-\U0001F5FF]"  # 各种符号和标志
            "|[\U0001F680-\U0001F6FF]"  # 交通和地图符号
            "|[\U0001F1E0-\U0001F1FF]"  # 国旗 (字母符号)
            "|[\U00002700-\U000027BF]"  # 杂项符号
            "|[\U0001F900-\U0001F9FF]"  # 补充符号
            "|[\U0001FA70-\U0001FAFF]"  # 补充符号和皮肤颜色
            "|[\U00002600-\U000026FF]"  # 杂项符号 (太阳、雪花等)
            "|[\U00002B50-\U00002B55]"  # 星号符号
        )

def tweets_embedding(each_user_tweets_path, community=2, dpo_epoch=None, output_path=None, device=0):
        if output_path is None:
            if dpo_epoch is None:
                output_path = f"/home/fanqi/llm_simulation/data/processed_data/community_{comm}/human_data/tweets_tensor.pt"
            else:
                output_path = f"/home/fanqi/llm_simulation/data/processed_data/community_{community}/tweets_dpo_{dpo_epoch}_tensor.pt"
        else:
...
def extract_tweets_1(community, epoch=None, output_path=None):
    if output_path is None:
        if epoch is not None:
            output_path = f'/home/fanqi/llm_simulation/data/processed_data/community_{community}/id_tweet_dpo_{epoch}.json'
        else:
            output_path = f'/home/fanqi/llm_simulation/data/processed_data/community_{community}/id_tweet_llama.json'
    else:
        output_path = output_path
    if not os.path.exists(output_path):
        print("extracting each_user's tweets")
        id_tweet={i:[] for i in range(len(user_idx))}
        human_tweets = json.load(open(f"/home/fanqi/llm_simulation/data/raw_data/community_{community}/human_tweet.json",'r'))
        if epoch is not None: 
            bot_tweets=json.load(open(f"/home/fanqi/llm_simulation/data/raw_data/community_{community}/dpo_{epoch}_bot_tweet.json",'r'))
        else:
            bot_tweets=json.load(open(f"/home/fanqi/llm_simulation/data/raw_data/community_{community}/llama_bot_tweet.json",'r'))
...
if __name__ == "__main__":
    for comm in [10, 11]:
    #     output_dir = f"/home/fanqi/llm_simulation/data/processed_data/community_2/"
    #     each_user_tweets_path=output_dir + f'id_tweet_dpo_{comm}.json'
    #     tweets_embedding(each_user_tweets_path, community=comm, output_path = f"/home/fanqi/llm_simulation/data/processed_data/community_2/tweets_dpo_{comm}_tensor.pt")
        # continue
        path=f'/home/fanqi/llm_simulation/data/raw_data/community_{comm}/'
        ...
        output_dir = f"/home/fanqi/llm_simulation/data/processed_data/community_{comm}/human_data/"
        ...
        human_tweets = json.load(open(f"/home/fanqi/llm_simulation/data/raw_data/community_{comm}/human_tweet.json",'r'))
        bot_tweets=json.load(open(f"/home/fanqi/llm_simulation/data/raw_data/community_{comm}/bot_tweet.json",'r'))
        ...
        tweets_embedding(each_user_tweets_path, community=comm, output_path = f"/home/fanqi/llm_simulation/data/processed_data/community_{comm}/human_data/tweets_tensor.pt")
