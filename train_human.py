...
COMM = 0
processed_data_folder=f"/home/fanqi/llm_simulation/data/processed_data/community_{COMM}/human_data/"
...
def generate_data(gpu_id, queue, samples, model_path, model_discriminator):
    ...
    for _ in tqdm(range(samples)):
        replace_indexed_human_ids = random.sample(indexed_human_ids, batch_size)
        replaced_human_indices, replaced_human_ids = zip(*replace_indexed_human_ids)
        batch_prompt = []
        for bot_id in replaced_human_ids:
        ...
        with torch.no_grad():
            ...
            for each_response in batch_response_1 + batch_response_2:
                clean_texts = []
                for each_tweet in each_response:
            ...
            tweets_tensor_1[list(replaced_human_indices)] = response_tensor[:batch_size]
            tweets_tensor_2 = tweets_tensor.detach()
            tweets_tensor_2[list(replaced_human_indices)] = response_tensor[batch_size:]
            ...
            output_2 = model_discriminator(des_tensor,
                                    ...
                                    edge_index,
                                    edge_type)
            response_score_1 = output_1[list(replaced_human_indices)]
            response_score_2 = output_2[list(replaced_human_indices)]
...
if __name__ == '__main__':
    ...
    vanilla_LLM_replace_human_tweet_path = raw_data_folder + "vanilla_LLM_replace_human.json"
    if not os.path.exists(vanilla_LLM_replace_human_tweet_path):
        ...
        output = {
            "replaced_human_ids": replaced_human_ids,
            "replaced_human_indices": replaced_human_indices
        }
        id_replaced_tweets = {k: v for d in replaced_human_tweets for k, v in d.items()}
        with open(vanilla_LLM_replace_human_tweet_path, "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=4)
        with open(processed_data_folder + "id_replaced_human_tweet.json", "w", encoding="utf-8") as f:
    ...
    replaced_tweets_tensor_path = processed_data_folder + "replaced_tweets_tensor.pt"
    if not os.path.exists(replaced_tweets_tensor_path):
        tweets_embedding(each_user_tweets_path=processed_data_folder + "id_replaced_human_tweet.json",
                          output_path=processed_data_folder + "replaced_tweets_tensor.pt",
                          community=COMM, device=other_device)
    updated_tweets_tensor=torch.load(processed_data_folder + "replaced_tweets_tensor.pt").to(other_device)
    ...
    print(f"Free GPUs found: {free_gpus}")
    num_gpus = len(free_gpus)
    for epoch in range(2, training_epoch):
        print(f"=== epoch: {epoch} ===")

        '''use the sft model or dpo model to rewrite the replaced human tweet'''
        dpo_replaced_human_tweet_path = processed_data_folder + f"id_tweet_dpo_{epoch}.json"
        ...
        updated_tweets_tensor=torch.load(processed_data_folder + f"tweets_dpo_{epoch}_tensor.pt").to(other_device)
        last_tweets_tensor = inputs[1]
        inputs[1] = updated_tweets_tensor
        ...
        ''' DPO training '''
        dpo_model_path = f"/home/fanqi/llm_simulation/models/DPO/merged_ckp_{epoch}"
