...
class TwitterAgent(BaseAgent):
    ...
    def att2score(self, content): # use Textblob
        ...
        stance = random.choice(['Support', 'Neutral', 'Oppose'])
    ...
    def generate_prompt(self, current_time, env_description):
        if isinstance(env_description, dict):
            trigger_news = env_description['content'] if self.name in env_description['receiver'] else ""
        elif isinstance(env_description, str):
        ...
        if len(self.memory.messages):
            questions = self._fill_context_for_retrieval(trigger_news)
            questions = questions.split('\n')
            self.retrieved_memory = self.memory_manipulator.retrieve(questions, 3)
        else:
            self.retrieved_memory = ""

        prompt = '[INST]' + self._fill_prompt_template(trigger_news) + '[/INST]'
        return prompt

    def step(self, current_time: dt, env_description: str = "", response: str = "") -> Message:
        """
        ...
        prompt = self._fill_prompt_template(trigger_news)

        parsed_response, reaction, target, parent_id = response, None, None, None
        att, stance = self.att2score(response)
        reaction_type = 'post'
        ...
        if trigger_news!="":
            msg = Message(content=trigger_news, sender='News')
