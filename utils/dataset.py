...
class Twibot22(Dataset):
    ...
    def Build_Graph(self):
        ...
        path=self.root+'two_edge_index.pt'
        if not os.path.exists(path):
            ...
            edge_index=torch.load(self.root+"two_edge_index.pt").to(self.device)
            edge_type=torch.load(self.root+"two_edge_type.pt").to(self.device)
            print('Finished')
